...
Write-Host "Phase 4 - Disconnect mailboxes from EXCHFRST users and re-connect to TARGETDOM users..." -ForegroundColor White
Write-Host "*************************************************************************************" -ForegroundColor White
Write-Host

#EXCHFRST Exchange Session
...
foreach($user in $users)
{
    try
    {
        ...
        if($ADUserResult.msExchRecipientTypeDetails -ne "2")
        {
            ...
            #connect mailbox to TARGETDOM user - changed -linkedMasterAccount switch to name attribute!!! <-----
